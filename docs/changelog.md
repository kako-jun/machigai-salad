...
- **GIFプレビューモーダル**
  - GIF生成後にプレビューモーダルを表示（シェア前にアニメーションを確認）
  - 「シェアする」「ほぞんする」「とじる」の3ボタン
...
- **比較結果のアニメーションGIFシェア**
  - 比較画面に「アニメーションをシェア」ボタン（btn-action）を追加
  - 左右画像を1秒間隔・無限ループのGIFアニメーションに変換
  - gif.js（Web Worker）+ Floyd-Steinbergディザリングで256色品質補完
  - スライドoffset・四隅ワープをGIFに反映（ユーザーが見ている状態を再現）
