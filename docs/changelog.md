...
- **比較・位置合わせ画面で右コーナーを外へドラッグするとページが右にずれる問題を修正** (#40)
  - ワープ用4隅ハンドルは canvas 描画ではなく絶対配置の DOM 要素で、自由な位置調整のため画像パネルの `overflow:hidden` の外に置かれている
  - 右コーナーの `left` = `画像右端 + cornerOffset − HANDLE_HIT_RADIUS(22)`。横幅いっぱいの画像ではドラッグ前から右端がビューポート端ぴったりで、外へドラッグすると最大 `MAX_CORNER_OFFSET`(30px) はみ出す
  - ハンドルの `touch-action:none` はタッチ由来のパンは止めるがレイアウトのはみ出しは止めず、モバイルではレイアウトビューポートが拡大（実測 392→422px）。`cornerOffset` がリリース後も残るため拡大も居座り、ページ全体・ポップアップが右にズレていた
  - 修正: コンテンツ列（`app/page.tsx` の `mx-auto max-w-lg px-4`）に `overflow-x-clip` を付与。スマホでは列が全幅なので clip 境界がビューポート端になり、ドット（コンテンツ端＝端から内側）は欠けず画面外のはみ出しだけが消える。`hidden` ではなく `clip` を使い縦方向（上へ描画されるハンドル）は visible のまま
  - 紙の検出画面（`PaperCornersAdjustment`）は canvas 描画＋画像内クランプのため無関係（実タッチ入力で検証してもはみ出しゼロ）

- **スマホでモーダルが右にずれ・横スクロールが発生する問題を修正** (#39)
  - 全モーダルの中身が `mx-4 w-full max-w-sm` で、box-sizing:border-box でもマージンは幅の外に加算されるため総幅 = 100% + 32px だった
  - 画面幅が `max-w-sm`(384px) 未満のスマホでは `w-full` がビューポート幅のまま 32px 超過。中身がビューポートより広くなり `justify-center` が負オフセットになるため、中身が左にはみ出し→右側に横スクロール領域が生まれていた（体感としては「右にずれて見える」）
  - 修正: 横余白を中身のマージンからオーバーレイの `p-4` へ移動。中身は `w-full max-w-sm`（マージン無し）に
  - あわせて縦方向も堅牢化: オーバーレイを `overflow-y-auto`、パネルを `my-auto` にし、ビューポートより縦に長い場合（横向きスマホ等）でもフッタが見切れずスクロールできるように
  - 対象: ImageProcessor / ImageUpload / SavesPopup の計7モーダル

- **Android でクロスフェード動画の生成が写真によって失敗する問題を修正** (#34)
  - 出力解像度（幅・高さ）が奇数になると、Android の MediaCodec ハードウェアエンコーダが H.264 4:2:0 構成を拒否し、動画生成が失敗していた
  - 修正: 動画パス（`generateCrossfadeVideo`）のみ幅・高さを偶数に切り捨て。GIF / APNG は従来どおり
...
- **OpenCV.js を自前ホスティング化（外部CDN配信停止への対応）**
  ...
  - CDN フォールバック機構を削除（自前配信なので不要）
  - ロード失敗メッセージを「ページをもう1回ひらいてみてね」に変更
  - Service Worker の network-first キャッシュにより2回目以降はオフラインでも動作

- **mesh-warpの×型アーティファクトを修正**
  - 画像を重ね合わせて変形（四隅・中央ポインタ操作）中に薄い灰色の×型の線が出る問題を修正
  - 原因: 4三角形分割描画の `clip()` 境界アンチエイリアスによるカバレッジ不足
  - 修正: クリップパスを重心から各頂点方向へ1.5px拡張し、隣接三角形間の継ぎ目を埋める

- **LINE内蔵ブラウザ対応（Android/iOS統一仕様）**
  - LINE内蔵ブラウザを UA で検出し、「外部ブラウザで開く」誘導バナーを表示（UIはAndroid/iOS共通）
  - Android: ボタン押下で `?openExternalBrowser=1` を付与して外部ブラウザ起動
  - iOS: 公式に外部ブラウザ起動のJS手段がないため、ボタン押下で「Safariで開いてね」案内モーダルを表示。手順ガイド＋「URLをコピー」ボタン付き
  - 判定ロジックは `lib/detectLineInApp.ts` に分離

- **ダウンロードファイル名のユニーク化**
  - GIF/APNGのダウンロードファイル名にローカルタイムスタンプを付与（例: `machigai-salad-20260413143052.png`）
  - 連続ダウンロード時の上書き確認ダイアログを回避

- **2枚目の画像選択の安定化**
  - `setTimeout + input.click()` をやめ、「2まい目をえらぶ」ボタン付きモーダルに変更
  - ユーザー操作起点でファイル選択を開くため、ブラウザのポップアップブロックに引っかからない

- **2枚モードの保存・復元で1枚目の画像が失われる問題を修正**
  - 2枚目の画像処理時に `originalImage`・`lastCornersRef`・`imageSize` が上書きされ、保存データが両方とも2枚目になっていた
  - 1枚目の情報を専用 ref で保持し、保存・復元時に正しく使い分けるよう修正
...
- **シェア促進メッセージ**
  - フッターのシェアボタン上のラベルを「このアプリを共有」→「おもしろかったら ともだちにも おしえてね」に変更
...
- **2枚モード（別々の画像を比較）**
  - 「アルバムから えらぶ」タップ時に「1まい / 2まい」選択ポップアップを表示
  - 2まいモード: 各画像を個別に角検出→角調整→台形補正し、分割せず比較画面へ
  - `processImageNoSplit` をuseOpenCVフックに追加（台形補正のみ・分割なし）
  - StepIndicatorを2まいモード対応（1まい目→2まい目→くらべる）
  - SaveEntryに `twoImageMode` / `rightImageData` を追加（保存・復元対応）
  - ABテスト、錯視画像、スクリーンショット比較などのユースケースに対応
...
- **LocalStorage保存・復元機能**
  ...
  - 元画像・4隅座標・ズレ補正量・コーナーワープ・画像サイズをルートキー(`machigai-salad`)のJSONオブジェクト内に格納
