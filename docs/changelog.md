...
- **比較結果のAPNGシェア**
  - 比較画面に「シェア」ボタン（btn-action、シェアネットワークアイコン付き）を追加
  - 左右画像を1秒間隔・無限ループのアニメーションPNG（APNG）に変換
  - upng-jsによるフルカラー（ロスレス）エンコード（GIFの256色制限なし）
  - Web Share API対応端末ではファイル付きシェア、非対応ではPNGダウンロード
  - 二重クリック防止、生成エラー時のトースト通知
